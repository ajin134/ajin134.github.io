		...
		shoping:function(options){
			...
			var S={
				init:function(){
					...
					$('.J-shoping-body').on('click',$close,this.removeList);
				...
				addShoping:function(e){
					...
					var $target=$(e.target),
						...
						Y = $shop.offset().top;
						//addcarParent=$('.addcar').parent().parent(),
						//addcarimg=addcarParent.siblings().find('img').attr('src'),
						//addcarTitle=addcarParent.find('h3').html(),
						//addcarPrice=addcarParent.find('.price').html(),

						//moveimg='<div id="floatOrder"><img src="'+(options.addcarimg)+'" width="50" height="50" /></div>';

					if(dis){
						if ($('#floatOrder').length <= 0) {
							$('body').append('<div id="floatOrder"><img src="'+options.addcarimg+'" width="50" height="50" /></div>');
						};
						var $obj=$('#floatOrder');
						if(!$obj.is(':animated')){
							$obj.css({'left': x,'top': y}).animate({'left': X,'top': Y-80},500,function() {
								$obj.stop(false, false).animate({'top': Y-20,'opacity':0},500,function(){
									$obj.fadeOut(300,function(){
										$obj.remove();	
										$target.addClass('dis-click');
										var l=$('.J-shoping-list').length,
											num=Number($num.text());
										if(l<5){
											$body.prepend('<div class="J-shoping-list" data-id="'+id+'"><a href="#"title=""><img src="'+options.addcarimg+'" width="50"height="50"/></a><div class="J-shoping-list-a"><p>'+options.addcarTitle+'</p><p><span class="right"><em>'+options.addcarPrice+'</em></span></p></div><div class="baseBg J-shoping-close"></div></div>');
										};
										$num.text(num+1);
									});
								});
